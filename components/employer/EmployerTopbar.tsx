...
interface TopbarProps {
    pumpName: string;
    employerName: string;
    activePage: "create" | "sales" | "nozzles";
    pumpId: string;
    employerId: string;
    onLogout?: () => void;
}
            ...
            <header className="w-full bg-card border-b border-border p-3 sticky top-0 z-50">
                <div className="flex items-center justify-between max-w-7xl mx-auto">
                    ...
                    <nav className="flex items-center gap-2">
                        ...
                        <Link href={`/employer/${pumpId}/${employerId}/nozzles`}>
                            <Button
                                variant="ghost"
                                size="sm"
                                className={`px-3 py-2 rounded-md transition-colors ${activePage === "nozzles"
                                    ? "bg-primary/10 text-primary"
                                    : "text-foreground hover:bg-muted"
                                    }`}
                            >
                                ...
                                <span className="hidden sm:inline text-sm font-medium">Daily Sales</span>
                            </Button>
                        </Link>
